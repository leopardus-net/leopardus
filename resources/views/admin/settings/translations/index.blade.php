...
<div class="row page-titles">
    <div class="col-md-5 col-12 align-self-center">
        <h3 class="text-themecolor">{{ trans('translations.title') }}</h3>
        <ol class="breadcrumb">
    ...
    <div class="col-md-7 col-4 align-self-center">
        <div class="d-flex m-t-10 justify-content-end">
            <div class="d-flex m-r-20 m-l-10 hidden-md-down">
                <button data-toggle="modal" id="newRole" data-target=".bs-example-modal-sm" class=" waves-effect waves-light btn-success btn  pull-right m-l-10"><i class="ti-plus text-white"></i> @lang('translations.new-group')</button>
            </div>
        </div>
    </div>
</div>
...
<div class="alert alert-warning" role="alert">
  @lang('translations.warning-msg-export-import')
</div>
...
<div class="card">
    ...
    <ul class="nav nav-tabs customtab" role="tablist">
        <li class="nav-item"> <a class="nav-link active" data-toggle="tab" href="#groups" role="tab"><span class="hidden-sm-up"><i class="ti-home"></i></span> <span class="hidden-xs-down">@lang('translations.groups')</span></a> </li>
        
        <li class="nav-item"> <a class="nav-link" data-toggle="tab" href="#import" role="tab"><span class="hidden-sm-up"><i class="ti-user"></i></span> <span class="hidden-xs-down">@lang('translations.import')</span></a> </li>
        <li class="nav-item"> <a class="nav-link" data-toggle="tab" href="#export" role="tab"><span class="hidden-sm-up"><i class="ti-user"></i></span> <span class="hidden-xs-down">@lang('translations.export')</span></a> </li>
    </ul>
    <!-- Tab panes -->
    <div class="tab-content">
        <div class="tab-pane active" id="groups" role="tabpanel">
            <div class="p-20">
                ...
                <div class="alert alert-success success-publish" style="display:none;">
                    @lang('translations.done-publish-group') '<?php echo $group ?>'!
                </div>

                <div class="form-group">
                    <p>@lang('translations.groups-msg')</p>
                    <select name="group" id="group" class="form-control group-select">
                        <option value="">@lang('translations.chose-group')</option>
                ...
                @if($group)
                    <form action="<?php echo action('TranslationsController@postAdd', array($group)) ?>" method="POST"  role="form">
                        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                        <div class="form-group">
                            <label>@lang('translations.add-new-key-group')</label>
                            <textarea class="form-control" rows="3" name="keys" placeholder="@lang('translations.key-group-form-placehold')"></textarea>
                        </div>
                        <div class="form-group">
                            <input type="submit" value="@lang('translations.btn-add-keys')" class="btn btn-primary">
                        </div>
                    </form>
                    
                    <hr>
                    <div class="table-responsive">
                    <table class="table color-bordered-table info-bordered-table hover" data-form="deleteForm">
                        <thead>
                        <tr>
                        ...
                        <?php foreach ($translations as $key => $translation): ?>
                            <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                                <td class="bg-info text-white" style="border:none"><?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?></td>
                                <?php foreach ($locales as $locale): ?>
                                    ...
                                    <td>
                                        <a href="#edit"
                                           ...
                                           data-emptytext="@lang('translations.empty-text')"
                                           data-title="@lang('translations.enter-trans')"><?php echo $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' ?></a>
                                    </td>
                                <?php endforeach; ?>
                                <?php if ($deleteEnabled): ?>
                                    <td>
                                        <a href="<?php echo action('TranslationsController@postDelete', [$group, $key]) ?>"
                                           class="deletekey"><i class="icon-trash"></i></a>
                                    </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
                    ...
                    <h4 style="width: auto;float:left;padding-top: 10px;">
                        @lang('translations.total'): <?= $numTranslations ?>, 
                        @lang('translations.updated'): <?= $numChanged ?>
                    </h4>
                    <form style="display: initial;float:right" 
                    class="form-inline form-publish pull-right" method="POST" action="<?php echo action('TranslationsController@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="@lang('translations.lang-group-export')">
                        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                        <button type="submit" class="btn btn-info" data-disable-with="@lang('translations.export-btn-load').." >@lang('translations.export-btn')</button>
                        <a href="<?= action('TranslationsController@index') ?>" class="btn btn-default">@lang('translations.back-btn')</a>
                    </form>
        ...
        <div class="tab-pane  p-20" id="import" role="tabpanel">
            <div class="alert alert-success success-import" style="display:none;">
                Done importing, processed <strong class="counter">N</strong> items! Reload this page to refresh the groups!
            </div>
            <div class="alert alert-success success-find" style="display:none;">
                Done searching for translations, found <strong class="counter">N</strong> items!
            </div>
            <p>@lang('translations.import-all-groups-text')</p>
            <form class="form-import" method="POST" action="<?php echo action('TranslationsController@postImport') ?>" data-remote="true" role="form">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <div class="form-group">
                    <div class="row">
                        <div class="col-sm-4">
                            <select name="replace" class="form-control">
                                <option value="0">@lang('translations.add-new-translations')</option>
                                <option value="1">@lang('translations.remplace-translations')</option>
                            </select>
                        </div>
                        <div class="">
                        <button type="submit" class="btn btn-success btn-block"  data-disable-with="@lang('translations.importing-load')">@lang('translations.import-translations')</button>
                        </div>
                    </div>
                </div>
            </form>
            <hr>
            <p>@lang('translations.import-all-scan-text')</p>
            <form class="form-find" method="POST" action="<?php echo action('TranslationsController@postFind') ?>" data-remote="true" role="form" data-confirm="@lang('translations.confirm-scan-translations')">
                <div class="form-group">
                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                    <button type="submit" class="btn btn-info" data-disable-with="@lang('translations.scan-load')" >@lang('translations.scan')</button>
                </div>
            </form>
              
        </div>
        <div class="tab-pane  p-20" id="export" role="tabpanel">
            <div class="alert alert-success success-publish-all" style="display:none;">
                @lang('translations.done-export-all')
            </div>
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('TranslationsController@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="@lang('translations.export-all-confirm')">
                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                    <button type="submit" class="btn btn-primary" data-disable-with="@lang('translations.export-all-load')" >@lang('translations.export-all-btn')</button>
            </form>
        </div>
    </div>
</div>
...
@include('admin.settings.translations.modals.new-group')
@stop
    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?php echo action('TranslationsController@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?php echo action('TranslationsController@index') ?>';
                }
            });

            $('table[data-form="deleteForm"]').on('click', '.deletekey', function(e){
                e.preventDefault();
